...
[当前版本安装器](https://github.com/lijingda/netizen/releases/download/v0.8.8/install.sh)；
已有应用的权限也会在激活前检查。
...
不确定下一步时发送 `/help` 查看快速开始。直接发送任务但还没有当前会话时，机器人会根据
项目和当前聊天的会话情况提示准备步骤；这条任务尚未执行，准备好后需要重新发送。

默认情况下，群聊和群话题中的每条请求都需要重新 **@机器人**，包括命令；实验性的
[自主模式](#自主模式实验)可按会话选择。`/new` 只通过卡片创建会话，
...
本实验分支提供可选的自主模式，默认关闭：群聊主线或普通群话题中的真人未 @ 消息，先由
外部决策模型判断是否交给 Codex。判断可能误触发或漏判；显式 **@机器人** 始终可走原有
处理入口，控制命令仍按现有规则使用。

实例管理员在 **Admin Web → 自主模式 · 实验** 配置 Jev 或已自行部署的 Laya，保存后下一次
判断生效；随后在群聊 `/new` 或空闲时的 `/config` 中选择“自主模式（实验）”。这两个会话
入口只选择模式，不填写模型连接或密钥。尚未配置时，确认提交会提示先去 Admin 配置，
不会创建半可用会话或改变原有设置。不要把 API Key 发到聊天中。

候选消息即使最终被跳过，也会先发送给所配置的决策服务；这里只说明一次数据去向，运行时
不会逐条通知。跳过的聊天不会自动补进 Codex，Agent 需要时自行[查询历史](#按需读取飞书历史)。
决策服务故障时未 @ 消息不自动执行、不积压补跑，显式 @ 不受该故障影响。

安装不增加配置问答，也不下载模型或部署 Laya；安装、升级、普通启动不要求决策服务在线。
不需要此模式就无需配置，原有功能照常使用。正式版本是否提供此实验功能，以当前实例页面为准。
[完整使用说明](skills/netizen-user-guide/references/user-guide.md#自主模式实验)
