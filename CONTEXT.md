...
**Autonomous Mode / 自主模式（实验）**：普通群会话中，由可替换决策模型选择未 @ 的真人消息，
再交给既有执行流程的可选触发方式。显式 @ 与控制入口不由决策模型否决；它不是 @ 上下文模式。

**Decision Context / 决策上下文（实验）**：某一会话中用于判断下一条候选消息的已消费输入、最终回复与摘要。
不包含被跳过的候选正文，也不是原生会话历史或自动补齐的群聊历史。
...
Card 折叠过程；最近进展和操作的原生事件时间由 Card 2.0 客户端按查看者时区与语言本地化。
commentary、plan step 等自由文本及随分页携带的过程 manifest 经过同一套有界敏感模式过滤；
工具名按原值显示并在 Markdown 边界转义。它不保存执行历史，也不是终态事实源。
