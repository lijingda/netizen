...
Completion Mention / 结束提及：原卡终态更新时首次加入真实 @，没有运行卡时在最终
富文本或文件卡中加入 @，不为提醒额外发送消息。选择原卡更新以保持单份结果，但
“更新时首次 @ 是否产生客户端通知”必须以真实客户端验证，不能从 API 成功推断。
...
提及是 Result 的一次投递元数据，正文保持原样，不增加第五种 Reply Card 模块。
运行投影没有该字段；终态渲染新增独立真实 at 节点。v4/v5 文件 manifest 只保留结果
正文，不携带提及身份；进程内保留的 Goal 控制投影也清除该字段。翻页或后续控制重绘
因此不会重新加入 @，原先的 @ 行可能随重绘消失。不新增通知队列、持久消息身份、
用户身份历史或跨重启补发。原有有界终态回退仍适用，展示异常不改变原生执行。
SDK 失败结果不能证明零投递。原卡更新或新卡发送已尝试提及后，后续卡片/正文回退
不再携带提及；没有目标或渲染失败等明确未投递的情况仍保留提及。Goal 超大正文先
更新不含 Result 的控制卡时，尚未尝试提及，后续独立正文仍可提及。只沿现有 Presenter
返回值区分未尝试与未确认，不增加去重状态。长富文本可能首段已提及、后续分段才被
审核拒绝，因此固定审核失败回执也不再次提及。
...
schema 升为 v11。服务严格只接受当前 schema；安装器在 manager target 已卸载、持有
...
私聊、群主线、话题的首次终态 @ 通知和后续重绘不重复通知；未实测前只承诺生成真实
提及，不宣称客户端通知已经通过。
