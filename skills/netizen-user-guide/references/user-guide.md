...
普通会话和 Goal 中，Codex 发出结构化问题时，机器人会为每题发送一张“Codex 提问”卡片。
选择一个建议，或选择“自行填写”并输入回答，再点击“提交回答”。选择建议时只提交该选项；
只有选择“自行填写”时才提交文本框内容。
自由填写最多 1,000 字符，较长回答可以直接在原会话发送。关闭进度卡不关闭问题卡片。

- 问题卡片始终属于发出问题的原会话。如果后来切换了会话，提交时会提示先通过
...
- 已归档、已删除、停止中、压缩中或不可用的会话沿用普通消息的限制。Side 临时话题当前
  不提供这类问题卡片。原生审批和其他需要宿主处理的旧式提问也不转成这类卡片。
...
- 普通会话和 Goal 的结构化提问使用飞书选择/填写卡片，回答按普通消息消费；它不是原生
