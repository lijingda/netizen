...
“立即运行”按计划当前保存的指令和会话配置执行一次，结果交付到计划目标会话的新话题。
它不改变原定时安排、下次执行时间或启停状态；暂停、已结束的计划也可手动运行。
重新启用只恢复未来自动触发，不等于立即运行。回执“已受理”表示触发成功，不表示任务
完成；最近执行和新话题会标明“手动触发”或“定时触发”。上次首轮仍在执行、状态不明，
或 Project 不可用时暂不能再次运行。Admin 响应未知时先查看最近记录；卡片原样重试和
...
- 同一计划上一次首轮尚未结束时，定时触发跳过、手动触发拒绝；首轮结束后的人工交流不会阻止后续
