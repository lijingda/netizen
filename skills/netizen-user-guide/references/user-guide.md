...
- Progress Card 开启后，普通或 Side Turn 被接受时回复一张运行卡；Goal 始终使用一张组合卡，
  ...
  时区与语言显示。读取、列举和搜索操作可显示原生路径、关键词和范围；无法从原生分类中
  取得对象信息的命令直接显示命令预览，失败命令可附带原生非零退出码。文件修改显示路径和
  变更类型，网页操作显示查询或链接，MCP/dynamic tool 继续显示原生工具名。
  操作预览最多 160 字符，文件和多查询最多预览前三项；进展文字保留普通路径、链接、邮箱和
  代码片段，明确凭据仍会过滤。不会根据命令名猜测操作目的，也不展示工具参数/结果、命令
