...
- 提交和接收内容：[消息、引用与图片](#发送消息引用与图片)、[本轮文件](#查看和发送本轮文件)。
...
3. 普通任务执行中继续发送消息，会补充到当前任务，不会排队成下一轮；
   Goal、停止或压缩状态会拒绝普通消息。
...
- 「结束时 @ 提醒」开启时，任务结束在最终回复中 @ 本轮任务发起人，执行中追加消息
  不改变提醒对象。开进度卡时在原卡最终更新中加入 @；关进度卡时在最终富文本或
  文件卡中加入 @，不额外发一条提醒。可确认的执行失败也提及；主动停止、中断和状态未知时
  不提及。Goal 在整项任务完成或因阻塞、额度/预算限制结束自动执行时提及一次，
  不逐个内部轮次提及，主动暂停不提及。文件翻页和后续卡片控制重绘不再次加入 @。
  卡片已尝试提及但投递结果未确认时，后续结果回退不重复 @。当前 Side 执行错误可能
...
- Progress Card 关闭时严格保持普通或 Side Turn 原有终态：无文件时使用富文本/静态文本回复，
  ...
  压缩不使用这些选项。自动定时首轮没有人类消息发起人，因此不 @；后续人类消息
  发起的任务正常使用结束提及设置。
...
- 当前请求只支持文本、普通图片和富文本图片。发送文件、音视频或在当前富文本中夹带
  不支持的附件时，整条请求会被拒绝，不会丢弃附件后仅执行文字。
- 引用或补充历史中的卡片图片、合并转发图片、文件和音视频只保留公开可见的资源信息，
  不读取二进制内容；其中的普通图片、富文本图片仍按上面的规则准备视觉输入。
...
Sessions、Side Topics 与定时计划。它与飞书共用同一个 Registry/Runtime，但权限来自独立 Admin
...
点击聊天或话题名称可打开对应飞书会话。Side Topics 页可按 Project、chat 与 route 状态
...
- “结果未确认，需要修复”时，两种维护提交都会受阻。请部署者以同一安装用户重新运行
...
计划支持与 `/new` 相同的模型、思考强度、速度、执行中表情闪烁、过程卡、结束时 @ 提醒和消息读取范围。
