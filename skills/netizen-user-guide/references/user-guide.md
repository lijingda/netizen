...
- 「结束时 @ 提醒」开启时，任务结束 @ 本轮任务发起人，执行中追加消息
  不改变提醒对象。开进度卡时，卡片更新完成后，在结果卡下另发一条话题 @ 回复；
  普通聊天以卡片而非用户消息建立话题，已有话题则留在原话题。新建的话题不继承原
  会话上下文。关进度卡时仍在最终回复中加入 @，不额外发提醒。可确认的执行失败也提及；主动停止、中断和状态未知时
  不提及。Goal 在整项任务完成或因阻塞、额度/预算限制结束自动执行时提及一次，
  不逐个内部轮次提及，主动暂停不提及。文件翻页和后续卡片控制重绘不再次加入 @。
  若过程卡不可用而另发最终回复，就直接在这条新回复中 @，不额外造卡或再发提醒。
  Goal 正文过长而另行回复时也相同。会话不支持话题或提醒投递结果未知时，不换位置
