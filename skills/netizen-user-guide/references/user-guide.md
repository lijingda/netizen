...
- Progress Card 开启后，普通或 Side Turn 被接受时回复一张运行卡；Goal 始终使用一张组合卡，
  ...
  当前状态、最近完成的 commentary、受限操作信息、子任务聚合与原生 checklist 的变化更新
  同一张卡。最近进展和操作使用显示到分钟的原生事件时间，并由每位飞书查看者的客户端按本地
  时区与语言显示；命令按 typed action 显示读取文件、列出文件、搜索内容、复合命令或通用执行命令，
  MCP/dynamic tool 显示 exact 工具名。工具名只做 Markdown 转义；commentary 等自由文本中的
  常见 secret/token、邮箱、用户目录和内联代码/参数会被过滤。不显示命令正文、参数、输出、
  server、路径、耗时、完成百分比、ETA 或内部 reasoning。终态会在同一
  卡片折叠过程并显示最终回答和
...
- 普通、Side 和 Goal 最后一轮成功执行都会按成功修改累计 `+N -M`，包括本轮新建子任务
  ...
  原文计 `+2 -2`。顶部总计可能包含已删除文件的行数，普通非图片文件行显示各自的数字。
...
结束临时 Side 会话，升级后不会自动续跑。关闭页面不会取消安装；重启后重新登录查看
安装器结果，重新连通不代表升级成功。源码安装仍在工作区运行 `./dev-install.sh`。
遇到“需要处理后重试”先完成配置或飞书权限；“结果未确认，需要修复”则按官方部署
说明运行安装器恢复，不能重复点击或自行删除恢复文件。
...
定时任务已接入当前工作区 V1，已验证私聊、普通群和话题群的执行投递，尚未正式发布。已安装实例是否提供
此入口，以其 `/help` 和 Admin 页面为准。

可直接在群聊或私聊中创建。比如在群里说：“每天北京时间九点，在当前群总结 test 项目的进展。”也可说
“列出这个群的定时任务”“把周报改到周五下午五点”“暂停周报计划”。省略目标 chat_id 和
...
再选择任务，在原卡片查看详情、编辑、启停、删除或展开最近执行；执行话题可通过飞书搜索定位。
卡片每批提供最多 50 个任务选项，可翻页；筛选切换会清除旧选择，编辑和启停后保留仍符合筛选的任务。
“刷新任务”只在选中任务后出现，用于重新读取该任务的信息；重新确认筛选可更新任务列表。
新建入口独立放在管理卡底部。新建和编辑使用一张完整表单，填写名称、指令、频率及对应时间、
Project 与会话配置后直接提交，取消返回管理卡。时间字段旁标明适用频率，均按所填时区解释；
校验失败保留填写内容，陈旧表单须刷新。私聊和群聊采用相同的创建流程。
Admin 的“定时任务”页提供相同管理能力，不直接发起即时执行。按 Project、启停状态和结束状态筛选，
默认显示全部 Project 中未结束的计划，包含已暂停计划。最后一次仍在启动、执行或结果待确认时也保留。
列表分别展示启停、是否结束和执行结果；已结束不表示执行成功。没有后续触发机会时不显示启停按钮，仍可编辑时间重新安排。
点击目标会话名称可以打开飞书聊天。创建/编辑在独立侧边表单中完成：Project 从列表选择，
一次性日期时间按表单中的时区填写，预览后保存；取消保留列表位置。目标会话仍填写 chat_id。

计划支持与 `/new` 相同的模型、思考强度、速度、执行中表情闪烁、过程卡和消息读取范围。
...
支持一次性、每天、每周和每 N 分钟四种规则，精度为分钟。新计划默认使用服务主机解析到的
IANA 时区，保存后不随主机时区变化；无法解析时须填写，例如 `Asia/Shanghai`。
...
截止时刻仍允许触发；截止不会停止正在执行的最后一次任务，它收尾后计划才显示已结束。

每次触发都新建独立飞书话题与普通持久会话，不带入创建计划时的聊天历史，也不切换
...
- 同一计划上一次自动首轮尚未结束时，本次跳过；首轮结束后的人工交流不会阻止后续
