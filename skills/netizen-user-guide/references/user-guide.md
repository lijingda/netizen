...
1. 发送 `/new`，在卡片中选择项目。不确定模型配置时可选择“继承 Codex”，也可选择
   ...
   默认关闭，可分别调整。
   群聊和群话题还可选择消息模式，包括可选的自主模式（实验）。`/new` 不接受参数。
...
如果还没有当前会话就直接发送任务，机器人会给出下一步：没有已启用项目时先去
`/settings`，有项目时通过 `/new` 创建会话；当前聊天或话题已有会话记录时，还会提示
`/sessions` 和 `/sessions archived` 查找并切换或恢复。**被拦下的任务尚未执行，准备好
会话后请重新发送。** 系统不会自动创建项目、会话或重放这条任务。
...
默认情况下，群聊主线和群话题中，每一条触发机器人的消息都必须重新 `@机器人`。单聊以及单聊中的
话题无需 `@`。即使选择“自动带上期间的群聊讨论”，未 @ 的消息也只可能在下一次 @ 时
作为背景进入 Codex，不会让机器人自动响应。只有显式选择[自主模式（实验）](#自主模式实验)的
普通群聊会话，才会对真人未 @ 消息进行消费判断；群聊中的控制命令仍需 @。
...
| `/config` | 修改当前空闲会话的模型、思考强度、速度、任务反馈和群聊消息模式，后续新任务生效 | [会话设置](#session-settings) |
...
- 可直接提供飞书 2.0 卡片、合并转发或转发话题，也可在单聊/群聊主线中回复它们并提问；默认群聊和
  话题的触发仍需每条 @机器人，自主模式按下面的规则判断未 @ 消息。它们作为材料交给 Codex，材料里的 `/stop`、`/new`、
...
群聊主线和普通群话题的 @ 触发有两种消息范围，可在 `/new` 创建时选择，也可在会话
空闲时通过 `/config` 切换；[自主模式（实验）](#自主模式实验)是同一消息模式选项中的另一种选择，
不与“自动带上期间的群聊讨论”叠加：
...
此能力默认关闭，且只在提供该实验功能的版本中出现。不配置决策模型不影响原有模式。
它适用于群聊主线和普通群话题，不用于单聊、单聊话题或 Side。

**启用和配置：**

1. 请实例管理员打开 **Admin Web → 自主模式 · 实验**，选择 Jev 或自行部署好的 Laya，
   填写服务地址、模型和所需 API Key。超时与输入预算位于高级设置。
2. 保存并应用后，可测试已保存连接。保存使下一次判断使用新配置，无需重启；测试只发送
   固定公开样例，不发送聊天，也不证明群聊判断准确。
3. 在群聊 `/new` 卡片选择“自主模式（实验）”，或空闲时通过 `/config` 切换。
   会话表单只选择模式，不提供地址、密钥或配置向导。

如果尚未配置就确认创建，会提示先请管理员到 Admin 配置后重新创建；本次不创建会话。
`/config` 同理，保留原设置，不自动改成其他模式或事后补建。自然语言询问如何配置时，
Agent 应引导用户到 Admin，不代改配置文件或读取、代填凭据。不要将 API Key 发送到聊天。
Admin 密钥输入框从不回显已保存的值；同一提供方和地址下留空保留，显式勾选清除才移除；
更换提供方或地址需要重新录入，避免把原密钥发给另一个服务。

**消息行为：**

- 真人未 @ 消息由决策模型判断。消费后沿用原有任务流程：空闲开始 Turn，运行中补充当前
  Turn，不排队。显式 @ 的普通消息绕过决策判断，仍受原有执行准入限制；控制命令保持原规则。
- 跳过时不发群通知，也不显示处理进度。候选消息即使最终跳过，仍会发送给所配置的决策
  服务；外部提供方可能按自身策略留存请求。这是静态数据流说明，不是逐条通知。
- 决策上下文只保留实际接收的消息、最终回复及旧记录摘要，不收集被跳过的正文或过程卡。
  自主期间明确 @ 且被接收的交互也进入记录。新会话或已有会话首次开启时不补录旧历史；
  关闭后保留已有记录但停止积累，再开启不补录关闭期间的对话。
- 触发不会自动带上之前被跳过的讨论，也不会把决策摘要塞进 Codex。Agent 可通过已有
  `netizen-lark` 能力按需查历史，其运行环境仍需具备相应 CLI／Skills 和权限。
  能准确计数时，后续输入只附带两次消费间未提交的用户消息条数，不单独发群通知。
- 图片、文件等先用可用文字和类型说明判断，不额外识图或解析文件；消费后仍遵守原有
  输入支持范围，并非开启自主模式就获得新的附件能力。其他机器人未 @ 消息不参与自主判断。

**误判与故障：**

这是实验功能，可能误触发或漏判；不希望承担误判时，可随时在空闲会话切回 @ 模式。
决策模型不可用、输入超过预算或管理员清除配置时，未 @ 消息不自动消费，不积压、不在恢复后
补跑，也不逐条发群错误通知。显式 @ 和控制入口继续可用；Admin 和 `/status` 可查看不可用原因。
会话仍保留自主模式选择，服务恢复或重新配置后继续判断新消息。

安装、升级和普通启动均不要求决策服务在线，也不会自动下载模型或部署 Laya；安装只提供
可选提示，不增加必答或可跳过的模型配置问答。
...
- `/new`：打开唯一的新建卡片。Project 下拉框展示全部 enabled Projects，不由 Netizen
  ...
  默认关闭，Progress Card 默认开启，可独立选择；群聊和群话题还可选择消息模式。
...
Sessions、Side Topics、定时计划，以及可选自主模式的决策模型配置。它与飞书共用同一个 Registry/Runtime，但权限来自独立 Admin
...
- `/new` 卡片可以为新会话选择继承 Codex 或显式 Model、Effort 和 Speed；群聊和群话题
  还可选择消息模式（两个 @ 模式或自主模式）；Reaction Pulse 默认关闭，Progress Card 和结束时 @ 提醒默认开启，可独立调整。
- `/config` 原子修改当前会话后续新 Turn 的三项模型设置、三个 Task Feedback，以及群聊 @ 时
  读取的消息范围或自主模式开关。它不创建 Turn，也不能直接配置另一个会话；应先 `/resume`。
  自主模式依赖的实例级决策模型仅在 Admin 配置，不在这里填写。
...
默认 @ 模式下，群主线和群话题的每条触发消息都必须重新 `@机器人`。若当前
会话选择“自动带上期间的群聊讨论”，这些消息可能在下一次 @ 时作为背景进入 Codex，但
仍不会自己触发 Turn。自主模式则可能是模型判定跳过、漏判或服务暂不可用，可通过显式 @
补救，并查看 `/status` 或请管理员检查 Admin。还应确认机器人仍在群内，且飞书应用的可用范围
和消息权限已发布。
