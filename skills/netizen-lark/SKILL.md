---
name: netizen-lark
description: 使用本机 Netizen 的飞书应用 profile 接入 lark Skills，并用当前消息的 message_id 按需定位聊天或话题、查询之前的讨论。
---
...
将 `whoami` 换成对应 lark Skill 的业务命令，全程保留此调用前缀和 `--as bot`。
...
提示修复；权限和查询错误按对应 lark Skill 处理，不切换其他应用或用户身份。
